...
import {useDeletePupilsMutation} from "../../../api/pupils-api-req";

PupilsListDeleteConfirm.propTypes = {
    open: PropTypes.bool.isRequired,
    onClose: PropTypes.func.isRequired,
    pupilsId: PropTypes.string.isRequired,
};

export default function PupilsListDeleteConfirm({open, onClose, pupilsId}) {
    const [deletePupils, deletePupilsRes] = useDeletePupilsMutation();

    const handleDelete = () => {
        if (!pupilsId) return null;

        const promise = deletePupils({id: pupilsId})
            .unwrap()
            .then(() => {
                onClose();
            });
